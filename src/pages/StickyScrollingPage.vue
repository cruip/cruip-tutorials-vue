...
import StickyScrolling from '../components/StickyScrolling.vue'
...
<template>
  <main class="relative min-h-screen flex flex-col justify-center bg-slate-50 overflow-hidden supports-[overflow:clip]:overflow-clip">
    <div class="w-full max-w-6xl mx-auto px-4 md:px-6 py-24">
      ...
      <StickyScrolling class="max-w-md mx-auto lg:max-w-none lg:min-h-[var(--stick-items)] hidden">
        <div class="lg:sticky lg:top-0 lg:h-screen space-y-16 lg:space-y-0">

          <template v-for="section in sections">
            <section class="lg:absolute lg:inset-0 lg:z-[var(--stick-visibility)]" style="--stick-visibility:0;--stick-scale:0.8;">
              <div class="flex flex-col lg:h-full lg:flex-row space-y-4 space-y-reverse lg:space-y-0 lg:space-x-20">
                <div class="flex-1 flex items-center lg:opacity-[var(--stick-visibility)] transition-opacity duration-300 order-1 lg:order-none">
                  <div class="space-y-3">
                    ...
                    <h2 class="text-4xl text-slate-900 font-extrabold">{{ section.title }}</h2>
                    <p class="text-lg text-slate-500">{{ section.text }}</p>
                  </div>
                </div>
                <div class="flex-1 flex items-center lg:scale-[var(--stick-scale)] lg:opacity-[var(--stick-visibility)] transition duration-300">
                  <img width="512" height="480" :src="section.img" :alt="section.title" />
                </div>
              </div>
            </section>
          </template>
  ...
  <Footer />
  <Banner tutorialUrl="https://cruip.com/how-to-create-a-sticky-on-scroll-effect-with-javascript/" downloadUrl="https://github.com/cruip/cruip-tutorials-vue/blob/main/src/components/StickyScrolling.vue" />
</template>