            ...
            <div class="md:flex items-center md:space-x-4 mb-3">
              <div class="flex items-center space-x-4 md:space-x-2 md:space-x-reverse">
                ...
                <div class="flex items-center justify-center w-10 h-10 rounded-full bg-white shadow md:order-1">
                  ...
                  <template v-else>
                      <svg :class="[ item.type === 'close' ? 'fill-red-500' : 'fill-emerald-500' ]" xmlns="http://www.w3.org/2000/svg" width="16" height="16">
                        <path d="M8 0a8 8 0 1 0 8 8 8.009 8.009 0 0 0-8-8Zm0 12a4 4 0 1 1 0-8 4 4 0 0 1 0 8Z" />
                      </svg>                    
                  </template>
                </div>            
                <!-- Date -->
                <time class="font-caveat font-medium text-xl text-indigo-500 md:w-28">{{ item.date }}</time>
              </div>
              <!-- Title -->
              <div class="text-slate-500 ml-14"><span class="text-slate-900 font-bold">{{ item.author }}</span> {{ typeText(item.type) }}</div>
            </div>
            <!-- Card -->
            <div class="bg-white p-4 rounded border border-slate-200 text-slate-500 shadow ml-14 md:ml-44">{{ item.content }}</div>
          </div>
    </template>
  </div>
</template>